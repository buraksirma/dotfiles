-- Customized https://github.com/nekowinston/wezterm-bar to match with my preferences
...
wezterm.on("format-tab-title", function(tab, tabs, _panes, conf, _hover, _max_width)
	...
	local tabtitle = basename(tab.active_pane.title)
	...
	local space = not tab.is_active and " " or ""
	local title = space .. tabtitle .. pane_count .. C.p

	return {
		{ Background = { Color = i_bg } },
		{ Foreground = { Color = i_fg } },
		{ Text = index },
...
wezterm.on("update-status", function(window, pane)
	...
	local divider_bg = conf.resolved_palette.indexed[16]
	...
	if C.clock.enabled then
		...
		window:set_right_status(wezterm.format({
			...
			{ Text = " " .. basename(pane:get_foreground_process_name() or "") .. " " },
			{ Background = { Color = palette.ansi[5] } },
			{ Foreground = { Color = palette.background } },
			{ Text = "  " },
			{ Background = { Color = palette.tab_bar.background } },
